...
export default function Navbar() {
  // open register modal when user click on the sign up button in the dropdown menu
  const { openRegister, openLogin } = useAuthModal();

  //open dropdown menu
  const [open, setOpen] = useState(false);

  //fold dropdown when any part of the page is click aside the dropdown
    ...
    <nav className="fixed top-0 z-50 w-full h-18 lg:h-24 bg-white border-b border-gray-200">
      <div className="flex items-center justify-between h-full mx-auto w-[95%] md:w-[90%]">
        ...
        <div className="flex items-center gap-4 relative">
          ...
          {open && (
            <div
              className="absolute right-0 top-14 w-56 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden px-4 py-2"
              ref={menuRef}
            >
